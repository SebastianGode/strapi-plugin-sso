...
async function keycloakSignInCallback(ctx) {
  ...
  try {
    ...
    console.log("Includes super-admin-role: " + userResponse.data.realm_access.roles?.includes(KEYCLOAK_STRAPI_SUPER_ADMIN_ROLE))
    if (
      !userResponse.data.realm_access.roles?.includes(KEYCLOAK_STRAPI_SUPER_ADMIN_ROLE) &&
      !userResponse.data.realm_access.roles?.includes(KEYCLOAK_STRAPI_EDITOR_ROLE) &&
      !userResponse.data.realm_access.roles?.includes(KEYCLOAK_STRAPI_AUTHOR_ROLE)
    ) {
